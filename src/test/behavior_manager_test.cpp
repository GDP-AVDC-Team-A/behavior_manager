...
bool startTask(std::string task_name, int priority, std::string parameters) {
  ros::ServiceClient start_task_srv = n->serviceClient<aerostack_msgs::StartTask>("/drone1/start_task");
  aerostack_msgs::StartTask msg;
  //aerostack_msgs::TaskCommand task_cmd;
...
bool stopTask(std::string task_name) {
  ros::ServiceClient stop_task_srv = n->serviceClient<aerostack_msgs::StopTask>("stop_task");
  aerostack_msgs::StopTask msg;
...
bool simulateBehaviorFinished(std::string task_name, int cause, int delay_ms){
  ...
  aerostack_msgs::BehaviorActivationFinished msg;
...
TEST(BehaviorManagerTest0, Existing_Behavior) {
  ...
  status = status && simulateBehaviorFinished("TAKE_OFF", aerostack_msgs::BehaviorActivationFinished::GOAL_ACHIEVED, 100);
  status = status && startTask("FOLLOW_PATH", 2, "");
  status = status && simulateBehaviorFinished("FOLLOW_PATH", aerostack_msgs::BehaviorActivationFinished::INTERRUPTED, 100);
  //status = status && stopTask("FOLLOW_PATH");
  status = status && startTask("FOLLOW_PATH", 2, "");
  status = status && startTask("FOLLOW_PATH", 2, "");
  status = status && simulateBehaviorFinished("FOLLOW_PATH", aerostack_msgs::BehaviorActivationFinished::WRONG_PROGRESS, 100);
  //status = status && stopTask("FOLLOW_PATH");
  status = status && startTask("LAND", 2, "");
  status = status && simulateBehaviorFinished("LAND", aerostack_msgs::BehaviorActivationFinished::GOAL_ACHIEVED, 100);
  EXPECT_TRUE(status);
}
...
int main(int argc, char **argv){
  ...
  deactivate_behavior_pub = n->advertise<aerostack_msgs::BehaviorActivationFinished>("/drone1/behavior_activation_finished", 0);
